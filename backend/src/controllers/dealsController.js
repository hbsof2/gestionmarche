...
async function update(req, res) {
  ...
  try {
    ...
    await logActivity(pool, {
      ...
      actionType: "update",
      action: `تعديل صفقة: ${rows[0].reference}`,
      section: "deals",
      ipAddress: req.ip,
    });
    res.json(rows[0]);
...
async function remove(req, res) {
  try {
    ...
    await logActivity(pool, {
      ...
      fullName: req.user.full_name,
      actionType: "delete",
      action: `حذف صفقة: ${rows[0].reference}`,
      section: "deals",
      ipAddress: req.ip,
    });
...
async function updateDealItem(req, res) {
  ...
  try {
    ...
    const { rows: dealRows } = await pool.query("SELECT reference FROM deals WHERE id = $1", [req.params.id]);
    await logActivity(pool, {
      userId: req.user.id,
      username: req.user.username,
      fullName: req.user.full_name,
      actionType: "update",
      action: `تعديل مادة في صفقة: ${dealRows[0]?.reference || ""}`,
      section: "deals",
      ipAddress: req.ip,
    });
    res.json(rows[0]);
...
async function removeDealItem(req, res) {
  try {
    ...
    await logActivity(pool, {
      ...
      fullName: req.user.full_name,
      actionType: "delete",
      action: `حذف مادة من صفقة: ${dealRows[0]?.reference || ""}`,
      section: "deals",
      ipAddress: req.ip,
    });
    res.json({ deleted: rows[0].id });
