...
**Receipts (الوصولات) — first phase** — `ReceiptsPage.jsx` follows the standard `*Page`/`*List`/`*Form`/`*DeleteModal` shape, with the row-view action handled by an inline `ReceiptViewModal` defined in `ReceiptsPage.jsx` itself rather than a separate file (a receipt has no sub-resources yet to justify a full detail page like `DealDetail.jsx`). `ReceiptForm.jsx` is the interesting piece: creating a receipt is a 4-step cascading form (contractor+authority → deal → branch → date/reference), each step gated on the previous selection and fetched fresh via `receiptsService.js`:
...
- `controllers/receiptsController.js` — first phase of the receipts section (migrations `013`), backed by `deals`/`contractors`/`contracting_authorities`/`authority_branches` joins (`FULL_SELECT` constant shared by `getAll`/`getById`/`create`/`update`). Mounts three lookup sub-resources in `routes/receipts.js` **before** `/:id` (`/deals`, `/deal-branches/:dealId`, `/next-counter/:dealId`) that back the frontend's cascading form — see the Frontend section above. `create` generates `reference = "{deal.reference}-{counter}"` and calls `createDealSnapshot()` inside a `pool.connect()` transaction; `update` only allows changing `receipt_date`/`branch_id`. `receipt_items` doesn't exist yet, so `remove()` guards for it with `tableExists()`.
...
- Now that `receipts` is a real table, both `dealsController.js`'s `remove` and `authorityBranchesController.js`'s `deleteBranch` also check `SELECT COUNT(*) FROM receipts WHERE deal_id = $1` / `WHERE branch_id = $1` (in addition to their pre-existing `deal_branches`/`deal_items` checks) before allowing a delete.
