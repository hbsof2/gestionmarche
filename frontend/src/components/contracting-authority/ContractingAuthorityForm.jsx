...
export default function ContractingAuthorityForm({ authority, onSave, onCancel }) {
  const isEdit = Boolean(authority);
  const initialDate = parseDate(authority?.rc_date);

  const [form, setForm] = useState({
    name: authority?.name || "",
    wilaya: authority?.wilaya || "",
    commune: authority?.commune || "",
    nis: authority?.nis || "",
    nif: authority?.nif || "",
    rc_number: authority?.rc_number || "",
  ...
  const validate = () => {
    const errs = {};
    if (!form.name.trim()) errs.name = "اسم المصلحة المتعاقدة مطلوب";
    ...
    if (!rcDay || !rcMonth || !rcYear) errs.rc_date = "تاريخ السجل التجاري مطلوب بالكامل";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSave({ ...form, rc_date }, authority?.id);
    } catch (err) {
